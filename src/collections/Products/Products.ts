    ...
    hooks: {
        ...
        beforeChange: [addUser, async (args) => {
            if(args.operation === "create"){
                ...
                const createdPorduct = await stripe.products.create({
                    name: data.name,
                    default_price_data: {
                        currency: "USD",
                ...
                const updated: Product = {
                    ...data,
                    stripeId: createdPorduct.id,
                    priceId: createdPorduct.default_price as string,
                }

                return updated
            } else if(args.operation === "update"){
                const data = args.data as Product
                const updatedPorduct = await stripe.products.update(data.stripeId!,{
                    name: data.name,
                ...
                const updated: Product = {
                    ...data,
                    stripeId: updatedPorduct.id,
                    priceId: updatedPorduct.default_price as string,
                }
